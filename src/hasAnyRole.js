...
/**
 * Check if it has any the roles
 * @param {GuildMember} member - The member to add the role
 * @param {Collection<String,Role>|Array<String>} roles - The roles to check
 * @returns {Boolean}
 */
module.exports = function hasAnyRole(member, roles) {
    ...
    return addedRoles.some(roleId => memberRolesSet.has(roleId));
}